...
`Page[CompactDecision]`/`Page[Decision]` instead of falling back to the
implementation's `Page[CompactDecision] | Page[Decision]` union.
"""
...
from binnacle_core.domain.models import CompactDecision, Decision, Page


async def _client_relevant_narrows(client: Binnacle) -> None:
    default_projection = await client.relevant()
    assert_type(default_projection, Page[CompactDecision])

    compact = await client.relevant(projection="compact")
    assert_type(compact, Page[CompactDecision])

    full = await client.relevant(projection="full")
    assert_type(full, Page[Decision])
